...
export const ClickNeighbours = ({ cell, boardArray }) => {
  ...
  if (left) {
    OnCellClick({ cell: boardArray[column - 1][row], boardArray });
  }
  if (right) {
    OnCellClick({ cell: boardArray[column + 1][row], boardArray });
  }
  if (up) {
    OnCellClick({ cell: boardArray[column][row - 1], boardArray });
  }
  if (down) {
    OnCellClick({ cell: boardArray[column][row + 1], boardArray });
  }
  if (up && left) {
    OnCellClick({ cell: boardArray[column - 1][row - 1], boardArray });
  }
  if (up && right) {
    OnCellClick({ cell: boardArray[column + 1][row - 1], boardArray });
  }
